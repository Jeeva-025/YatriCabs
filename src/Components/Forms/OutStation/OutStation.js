...
import './OutStation.css'; // Import CSS for styling
...
function Outstation() {
  ...
  const [selectedButton, setSelectedButton] = useState('outstation');
 ...
 console.log(oneWay);
  return (
    ...
    <div className="home-container">
      ...
      <button
          className={`header-buttons ${selectedButton === 'outstation' ? 'selected' : ''}`}
      ...
      <button className={`sub-buttons ${oneWay ? 'enabled' : ''}`} onClick={() => setOneWay(!oneWay)}>One Way</button>
      <button className={`sub-buttons ${oneWay ? '' : 'enabled'}`} onClick={() => setOneWay(!oneWay)}>Round Trip</button>
      </div>
